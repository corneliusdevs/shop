...
const FlutterwaveHook = ({email, phoneNum, amount, name, userId, products, accessToken})=>{
  ...
  const config = {
    publicKey:"FLWPUBK_TEST-b7b4f63e6849f117a859574ffb619654-X",
    tx_ref: Date.now(),
  ...
  useEffect(()=>{
    const onSuccessfullyPaid = (response)=>{
    ...
    const processProducts = ()=>{
      ...
      axios.post("http://localhost:5000/api/orders", reqBody, {
        headers: {
   ...
   makePayment === true && handleFlutterwavePayment({
        callback: onSuccessfullyPaid,
